...
local function cacheable_request(method, uri, conf)
  -- Only GET method allowed for using Cache
  if method ~= "GET" then
    return false
  end

  -- Only matching URI(s) allowed for using cache
...
-- Build Cache key for the request, based on URI, headers, and/or queryParams given
...
function CacheHandler:access(conf)
  ...
  -- Check existing cache (if available)
  local cached_val, err = red:get(cache_key)
  local cached_val_header, err = red:get(cache_key.. ".header")
  if cached_val and cached_val ~= ngx.null then
    ngx.log(ngx.NOTICE, "cache hit")

    local val = json_decode(cached_val)    
    local headers = json_decode(cached_val_header)    
    for k,v in pairs(headers) do
      ngx.req.set_header(k, v)
    end

    -- Cache FOUND !! return immediately to end process
    ngx.header["content-encoding"] = "gzip"
    ngx.header["Accept-Encoding"] = "gzip"
    ngx.say(cached_val)
    return ngx.exit(200)

    -- Might need to use these response someday..
    -- return responses.send_HTTP_OK(val.content)
    --return responses.send_HTTP_OK(whateverhere) -- OK ????
    --ngx.header["Content-Type"] = "application/json; charset=utf-8"
  end

  -- If reached this far, then no error happened & no cache available
  ngx.log(ngx.NOTICE, "cache miss")

  -- flag response_cache with generated cache_key (mark this request as 'cacheable') -> *flag1
...
function CacheHandler:body_filter(conf)
  ...
  -- If response_cache has no value (flagged from *flag1) , skip this process altogether
  local ctx = ngx.ctx.response_cache
  if not ctx then
    return
  end

  -- Build cache value based on upstream response (concatenate chunk until eof)
  ...
  -- Only save to cache (using marked cache_key) when eof from upstream response is reached
  if eof then
    -- local content = json_decode(ctx.res_body)
    -- local value = { content = content, headers = ctx.headers }
    -- local value_json = json_encode(value)
    local contentVal = ctx.res_body
    local contentHeader = ctx.headers
    -- local contentObj = json_decode(ctx.res_body)
    -- local value = { contentV = contentVal, contentO = contentObj, headers = ctx.headers }
    ngx.timer.at(0, red_set, ctx.cache_key .. ".header", json_encode(contentHeader), conf)
    ngx.timer.at(0, red_set, ctx.cache_key, contentVal, conf)
  end
end
